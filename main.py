...
def extract_data(player_url):
    ...
    try:
        ...
        response = requests.get(SCRAPER_API_URL, params=params, timeout=60) # Increased timeout
        ...
        response.raise_for_status()  # This checks for other HTTP errors like 4xx/5xx
    ...
    def parse_any_currency(text):
        ...
        if not text:
            return None, 0.0
        # Finds a currency symbol (non-digit, non-whitespace) followed by a number.
    ...
    for row in offline_rows:
        try:
            ...
            # ROI calculation (only if buy-in and prize currency are the same and known)
